...
# Attempt to install FFmpeg automatically using winget on Windows
def install_ffmpeg():
    try:
        ...
        messagebox.showerror("Installation Failed", f"Automatic installation of FFmpeg failed.\nError: {str(e)}\n\nPlease install FFmpeg manually.")

# Custom rounded button widget using a Canvas for a modern look
...
class YouTubeDownloaderGUI:
    def __init__(self, root):
        ...
        # Setup window icon from logo image
        ...
        # Setup main content frame and styling
        ...
        # Progress bar and download button in control frame
    ...
    # Minimize the window to the system tray using pystray
    def minimize_to_tray(self):
        ...
        self.tray_icon = pystray.Icon("YT Grabber", image, "YT Grabber", menu)
    ...
    # Record initial click position for dragging window
    def click_title_bar(self, event):
        self._offset_x = event.x
        self._offset_y = event.y

    # Drag window based on mouse movement
    def drag_window(self, event):
        x = self.root.winfo_x() + (event.x - self._offset_x)
        y = self.root.winfo_y() + (event.y - self._offset_y)
        self.root.geometry(f"+{x}+{y}")

    # Exit the application and stop tray icon if active
    def exit_app(self, icon=None, item=None):
        if hasattr(self, 'tray_icon'):
            self.tray_icon.stop()
        self.root.destroy()

    # Update quality options based on selected format (video or audio)
    ...
    # Return format string for yt_dlp based on selected quality and format type
    ...
    # Update progress bar and status text during download
    ...
    # Download video/audio using yt_dlp with the provided options
    def download_video(self):
        ...
        # This script supports Windows only
        ...
        # Check for FFmpeg and attempt installation if not found
        ...
        # Add postprocessor based on selected format
        if self.format_type.get() == "video":
    ...
    # Reset download button and progress state
    ...
    # Toggle download start/stop based on current state
